...
def create_one_call(API_name, functions_to_sigs,
    ...
    if len(arg_id_to_constraints) > 0:
        ...
        for arg_id, constraints in arg_id_to_constraints.items():
            ...
            #if(invariant_sets[target_invariant_set_for_arg[arg_id]][1]==9919999999):
            if(target_invariant_set_for_arg[arg_id]==9919999999):
                print('matching_ways_id=9919999999')
                target_invariant_set_for_arg[arg_id] = 125
                matching_ways_id = invariant_sets[target_invariant_set_for_arg[arg_id]][1]
                print(matching_ways_id)
                outflag = 1

            elif(target_invariant_set_for_arg[arg_id]==9929999999):
                print('matching_ways_id=9929999999')
                target_invariant_set_for_arg[arg_id] = 125
                matching_ways_id = invariant_sets[target_invariant_set_for_arg[arg_id]][1]
                print(matching_ways_id)
                outflag = 2
            ...
            # if(outflag==1):
            # #if(len(matching_ways_id) == 9919999999):
            #     #sampled_matching_way = 9919999999
            #     sampled_matching_way = random.choice(matching_ways_id)
            #     print('[trying to pick way from the chosen targets output_encoding] selected way:', sampled_matching_way, ' for arg_id', arg_id)
            # else:
            sampled_matching_way = random.choice(matching_ways_id)
...
def create_one_test_and_run(target_function, all_functions,
    ...
    print(f'===========================START=================================')
    print(f'get_rules() in create_one_test_and_run')
    print(f'target_function: {target_function}')
    #print(f'all_function_sigs: {all_function_sigs}') indicate input argument
    print(f'target_arg: {target_arg}')
    print(f'solution_queue[target_function]: {solution_queue[target_function]}')
    print(f'inferred_queue[target_function]: {inferred_queue[target_function]}')
    print(f'already_started[target_function]: {already_started[target_function]}')
    print(f'seeds: {seeds}')
    #print(f'undominated_invariant_sets: {undominated_invariant_sets}')
    #print(f'history[target_function]: {history[target_function]}')
    #print(f'random_or_solved_choices[target_function]: {random_or_solved_choices[target_function]}')
    print(f'generate_random: {generate_random}')
    ...
    print('created_one_call', time.time())
    if not succeeded:
    ...
    if is_no_problemo_runs:
        ...
        for file in os.listdir(READ_DIRECTORY):
            ...
            each_score = float(file_parts[0] + "." + file_parts[1])
            score += each_score
            file_count += 1
            os.remove(READ_DIRECTORY+ "/" + file)
        average_score = score / file_count
        ...
        if average_score > 0:
            if score_max <= 0:
                score_max = average_score
                score_min = average_score
            if average_score > score_max:
                score_max = average_score
            if average_score < score_min:
                score_min = average_score
        ...
        if average_score > 0:
            ...
            if (score_max != score_min):
                normalized_s = (average_score - score_min) / (score_max - score_min)
        else:
            normalized_s = 1
...
def main():
    ...
    #undominated_invariant_sets get index from file
    ...
    try:
        ...
        #target_functions is target api for testinh
                    ...
                    while target_function_i < len(target_functions_sequence):
                        ...
                        with open(name, 'a') as all_outfile:
                            ...
                            is_crash, is_no_problemo_runs, is_problem, is_py_error, is_crash, outcome = create_one_test_and_run(
                                ...
                                already_ran,
                                test_i)
                            print('outcome from the outside:', outcome, '\t for function', target_function, 'i=',
                                  test_i)
