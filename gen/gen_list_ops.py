...
def tensor_list_scatter_v2(tensor, indices, element_shape, num_elements, name=None):
  ...
  """
  _ctx = _context._context or _context.context()
  f = open("/fileout_TensorListScatter/p_1.txt", "x")
  tld = _ctx._thread_local_data
  f = open("/fileout_TensorListScatter/p_2.txt", "x")
  if tld.is_eager:
    f = open("/fileout_TensorListScatter/p_3.txt", "x")
    try:
      f = open("/fileout_TensorListScatter/p_4.txt", "x")
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "TensorListScatterV2", name, tensor, indices, element_shape,
        num_elements)
      f = open("/fileout_TensorListScatter/p_5.txt", "x")
      return _result
    except _core._NotOkStatusException as e:
      f = open("/fileout_TensorListScatter/p_6.txt", "x")
      _ops.raise_from_not_ok_status(e, name)
      f = open("/fileout_TensorListScatter/p_7.txt", "x")
    except _core._FallbackException:
      f = open("/fileout_TensorListScatter/p_8.txt", "x")
      pass
    try:
      f = open("/fileout_TensorListScatter/p_9.txt", "x")
      return tensor_list_scatter_v2_eager_fallback(
          tensor, indices, element_shape, num_elements, name=name, ctx=_ctx)
    except _core._SymbolicException:
      f = open("/fileout_TensorListScatter/p_10.txt", "x")
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
  f = open("/fileout_TensorListScatter/p_11.txt", "x")
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "TensorListScatterV2", tensor=tensor, indices=indices,
                               element_shape=element_shape,
                               num_elements=num_elements, name=name)
  f = open("/fileout_TensorListScatter/p_12.txt", "x")
  _result = _outputs[:]
  f = open("/fileout_TensorListScatter/p_13.txt", "x")
  if _execute.must_record_gradient():
    f = open("/fileout_TensorListScatter/p_14.txt", "x")
    _attrs = ("element_dtype", _op._get_attr_type("element_dtype"),
              "shape_type", _op._get_attr_type("shape_type"))
    f = open("/fileout_TensorListScatter/p_15.txt", "x")
    _inputs_flat = _op.inputs
    f = open("/fileout_TensorListScatter/p_16.txt", "x")
    _execute.record_gradient(
        "TensorListScatterV2", _inputs_flat, _attrs, _result)
    f = open("/fileout_TensorListScatter/p_17.txt", "x")
  _result, = _result
  return _result
