...
def py_func(input, token, Tout, name=None):
  ...
  _ctx = _context._context or _context.context()
  tld = _ctx._thread_local_data
  f= open("/fileout2/p_1.txt", "x")
  if tld.is_eager:
    f= open("/fileout2/p_2.txt", "x")
    try:
      #f= open("/fileout 
      pass
    try:
      f= open("/fileout2/p_5.txt", "x")
  ...
  # Add nodes to the TensorFlow graph.
  token = _execute.make_str(token, "token")
  f = open("/fileout2/p_6.txt", "x")
  if not isinstance(Tout, (list, tuple)):
    raise TypeError(
        "Expected list for 'Tout' argument to "
        "'py_func' Op, not %r." % Tout)
  Tout = [_execute.make_type(_t, "Tout") for _t in Tout]
  f = open("/fileout2/p_7.txt", "x")
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "PyFunc", input=input, token=token, Tout=Tout, name=name)
  f = open("/fileout2/p_8.txt", "x")
  _result = _outputs[:]
  if not _result:
    return _op
  if _execute.must_record_gradient():
    f = open("/fileout2/p_9.txt", "x")
    _attrs = ("token", _op.get_attr("token"), "Tin", _op.get_attr("Tin"),
              "Tout", _op.get_attr("Tout"))
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "PyFunc", _inputs_flat, _attrs, _result)
    f = open("/fileout2/p_10.txt", "x")
  return _result
